 ...
 *   <li>Write .idea configuration files (modules.xml, misc.xml, vcs.xml, .gitignore)</li>
 *   <li>Write Eclipse-linked .iml files for each module</li>
 *   <li>Configure code style</li>
 *   <li>Copy run configurations from source project</li>
 *   <li>Copy compiler settings from source project</li>
 *   <li>Open the fully configured project</li>
 * </ol>
 * <p>
 * The project is configured <b>before</b> opening, so IntelliJ loads it with all
 * modules already registered.  Each module's {@code .iml} file uses
 * {@code classpath="eclipse"} which tells IntelliJ to read the Eclipse
 * {@code .classpath} and {@code .project} files directly — no Eclipse import
 * wizard API is needed.
 */
public class AmosProjectCheckoutAndImportTask extends Task.Backgroundable {
    ...
    /**
     * modules.xml — declares all 5 modules with .iml files in each module's own directory.
     * Matches the reference project at E:\ws\amos-25.12\.idea\modules.xml.
     */
    private static final String MODULES_XML =
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
            "<project version=\"4\">\n" +
            "  <component name=\"ProjectModuleManager\">\n" +
            "    <modules>\n" +
            "      <module fileurl=\"file://$PROJECT_DIR$/amos_client/amos_client.iml\" filepath=\"$PROJECT_DIR$/amos_client/amos_client.iml\" />\n" +
            "      <module fileurl=\"file://$PROJECT_DIR$/amos_server/amos_server.iml\" filepath=\"$PROJECT_DIR$/amos_server/amos_server.iml\" />\n" +
            "      <module fileurl=\"file://$PROJECT_DIR$/amos_shared/amos_shared.iml\" filepath=\"$PROJECT_DIR$/amos_shared/amos_shared.iml\" />\n" +
            "      <module fileurl=\"file://$PROJECT_DIR$/amos_test_junit/amos_test_junit.iml\" filepath=\"$PROJECT_DIR$/amos_test_junit/amos_test_junit.iml\" />\n" +
            "      <module fileurl=\"file://$PROJECT_DIR$/amos_web/amos_web.iml\" filepath=\"$PROJECT_DIR$/amos_web/amos_web.iml\" />\n" +
            "    </modules>\n" +
            "  </component>\n" +
            "</project>\n";
    ...
    /**
     * misc.xml — project SDK and language level.
     * Matches the reference project configuration.
     */
    ...
    /** vcs.xml — Git VCS mapping for the project root. */
    ...
    public void run(@NotNull ProgressIndicator indicator) {
        ...
        if (alreadyCheckedOut) {
            ...
            boolean resuming = Files.isDirectory(checkoutDir.resolve(".git"));
        ...
        // Step 2 — Write ALL .idea configuration files BEFORE opening the project.
        //
        // This is the key change: instead of opening an empty project and then
        // trying to import modules via the Eclipse import API, we write:
        //   - .idea/modules.xml        (declares all 5 modules)
        //   - .idea/misc.xml           (JDK, language level)
        //   - .idea/vcs.xml            (Git VCS mapping)
        //   - .idea/.gitignore         (standard IntelliJ defaults)
        //   - .idea/codeStyles/codeStyleConfig.xml
        //   - <module>/<module>.iml    (Eclipse-linked .iml for each module)
        //
        // When IntelliJ opens the project, it reads modules.xml, finds the .iml
        // files, and each .iml says classpath="eclipse" — so IntelliJ reads the
        // Eclipse .classpath/.project files directly.  No import wizard needed.
        indicator.setText("Writing project configuration…");
        try {
            writeProjectConfiguration();
        } catch (IOException e) {
            LOG.error("Failed to write project configuration", e);
        ...
        // Step 3 — Copy Run Configurations (before opening project)
        ...
        // Step 4 — Copy Compiler Settings (before opening project)
        if (sourceProjectPath != null) {
        ...
        // Step 5 — Open the fully configured project
        ...
        ApplicationManager.getApplication().invokeAndWait(() -> {
            try {
                ...
                projectRef[0] = com.intellij.ide.impl.ProjectUtil.openOrImport(
                        checkoutDir.toString(), null, false);
            } catch (Exception e) {
                LOG.error("Failed to open project at " + checkoutDir, e);
            }
        }, ModalityState.any());

        if (projectRef[0] == null) {
            LOG.error("ProjectUtil.openOrImport returned null for " + checkoutDir);
            ApplicationManager.getApplication().invokeLater(() ->
                    Messages.showErrorDialog(
        ...
        // Step 6 — Wait for post-startup initialization
        indicator.setText("Waiting for project initialization…");
        waitForProjectReady(projectRef[0], indicator);
        if (indicator.isCanceled()) return;
        indicator.setFraction(0.90);

        // Step 7 — Verify modules were loaded correctly
        indicator.setText("Verifying module configuration…");
        verifyModules(projectRef[0]);

        indicator.setFraction(1.0);
        LOG.info("AMOS project setup complete: " + checkoutDir);
    }

    // =========================================================================
    // Step 2 — Write Project Configuration
    // =========================================================================

    /**
     * Writes all IntelliJ project configuration files to the {@code .idea/} directory
     * and Eclipse-linked {@code .iml} files into each module directory.
     * <p>
     * After this method completes, the checkout directory is a fully configured
     * IntelliJ project that can be opened directly — no import wizard is needed.
     */
    private void writeProjectConfiguration() throws IOException {
        Path ideaDir = checkoutDir.resolve(".idea");
        Files.createDirectories(ideaDir);

        // .idea/modules.xml — declares all 5 modules
        Files.writeString(ideaDir.resolve("modules.xml"), MODULES_XML);
        LOG.info("Written modules.xml with " + MODULE_NAMES.size() + " modules");

        // .idea/misc.xml — JDK and language level
        // Only write if not already present (source project copy may provide a better one)
        Path miscXml = ideaDir.resolve("misc.xml");
        if (!Files.exists(miscXml)) {
            Files.writeString(miscXml, MISC_XML);
            LOG.info("Written misc.xml (JDK 21)");
        }

        // .idea/vcs.xml — Git VCS mapping
        Files.writeString(ideaDir.resolve("vcs.xml"), VCS_XML);
        LOG.info("Written vcs.xml (Git)");
        ...
        // Eclipse-linked .iml files — one per module
        List<String> written = new ArrayList<>();
        List<String> skipped = new ArrayList<>();
        for (String moduleName : MODULE_NAMES) {
            Path moduleDir = checkoutDir.resolve(moduleName);
    ...
    private boolean isAlreadyCheckedOut() {
        if (!Files.isDirectory(checkoutDir.resolve(".git"))) return false;

        // Sanity check: at least one expected module directory should exist
        boolean hasModuleDir = MODULE_NAMES.stream()
                .anyMatch(m -> Files.isDirectory(checkoutDir.resolve(m)));
        ...
        try {
            ...
            cmd.setWorkDirectory(checkoutDir.toFile());
            ...
            if (output.getExitCode() == 0) {
                ...
                LOG.info("Checkout dir on branch '" + currentBranch + "' but expected '" + branch + "' — will resume");
            }
        } catch (Exception e) {
    ...
    private boolean step1Clone(@NotNull ProgressIndicator indicator) {
        ...
        // ------------------------------------------------------------------
        boolean isResume = Files.isDirectory(checkoutDir.resolve(".git"));

        if (isResume) {
            LOG.info("Existing .git found — resuming (fetch + checkout) for branch " + branch
                     + " (user=" + user + ")");
        } else {
    ...
    private ProcessOutput runClone(@NotNull String gitExe,
        ...
        String cloneUrl = AmosCredentialStore.injectCredentials(repoUrl, user, pass);
        ...
        cmd.addParameters("clone",
                ...
                cloneUrl, checkoutDir.toString());

        cmd.setRedirectErrorStream(false);

        LOG.info("Clone command: git clone --single-branch --branch " + branch + " " + repoUrl
                 + " → " + checkoutDir + " (credentials " + (user.isEmpty() ? "NONE" : "provided") + ")");
    ...
    private ProcessOutput runResume(@NotNull String gitExe,
        ...
        try {
            ...
            setUrl.setWorkDirectory(checkoutDir.toFile());
        ...
        indicator.setText2("Fetching from remote…");
        {
            ...
            fetchCmd.setWorkDirectory(checkoutDir.toFile());
            fetchCmd.addParameters("fetch", "origin", branch, "--progress");
        ...
        indicator.setText2("Checking out branch " + branch + "…");
        {
            GeneralCommandLine checkoutCmd = buildGitCmd(gitExe, authHeader);
            checkoutCmd.setWorkDirectory(checkoutDir.toFile());
            // Use -B so it works whether or not the local branch already exists
            checkoutCmd.addParameters("checkout", "-B", branch, "origin/" + branch);
    ...
    private boolean hasLfsFiles() {
        if (Files.isDirectory(checkoutDir.resolve(".git").resolve("lfs"))) return true;
        Path gitattributes = checkoutDir.resolve(".gitattributes");
    ...
    private void step5CopyRunConfigs() {
        assert sourceProjectPath != null;

        Path srcRunConfigs = sourceProjectPath.resolve(".idea").resolve("runConfigurations");
        ...
        Path destRunConfigs = checkoutDir.resolve(".idea").resolve("runConfigurations");
        try {
    ...
    private void step6CopyCompilerSettings() {
        ...
        Path srcIdeaDir = sourceProjectPath.resolve(".idea");
        Path destIdeaDir = checkoutDir.resolve(".idea");
    ...
    private String substituteProjectDir(@NotNull String content) {
        ...
        String nativePath = sourceProjectPath.toString();
